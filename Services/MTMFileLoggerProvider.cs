...
/// </summary>
public class MTMFileLoggerProvider : ILoggerProvider
{
...
internal class MTMFileLogger : ILogger
{
    ...
    public void Log<TState>(LogLevel logLevel, EventId eventId, TState state, Exception? exception, Func<TState, Exception?, string> formatter)
    {
        ...
        // Determine if this is a MySQL-related log
        if (IsMySqlRelated(message, exception, _categoryName))
        {
            _fileLoggingService.LogMySql(logLevel, message, GetMySqlOperation(message), exception, context);
        }
        else
        {
            // Route to business log
            var category = DeterminBusinessCategory(_categoryName, message);
            _fileLoggingService.LogBusiness(logLevel, message, category, context);
        }
    }
    ...
    private static bool IsMySqlRelated(string message, Exception? exception, string categoryName)
    {
        ...
        if (exception is MySqlException || exception is InvalidOperationException && exception.Message.Contains("MySQL"))
            return true;
    ...
    /// </summary>
    private static string DeterminBusinessCategory(string categoryName, string message)
    {
