...
public static class StartupDialog
{
    ...
    private static string GetLogPath(IConfiguration configuration)
    {
        ...
        // Default fallback - use configuration-based path instead of hardcoded
        var defaultServerPath = configuration["Logging:DefaultServerPath"] ??
                               Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.CommonApplicationData), "MTM", "Logs");
        return Path.Combine(defaultServerPath, Environment.UserName.ToUpper());
    }
    ...
    private static async Task ShowStartupMessageAsync(string title, string message)
    {
        try
        {
            ...
            switch (applicationLifetime)
            {
                ...
                default:
                    // For mobile platforms and unknown application types, fall back to console output
