...
- stage: ThemeValidation
  ...
  - job: ValidateThemeStructure
    ...
    - task: PowerShell@2
      ...
      inputs:
        targetType: 'filePath'
        filePath: 'scripts/theme-management/validate-theme-structure.ps1'
  ...
  - job: ValidatePerformance
    ...
    - task: PowerShell@2
      ...
      inputs:
        targetType: 'filePath'
        filePath: 'scripts/performance-optimization/performance-test-themes.ps1'
        arguments: '-ThemeDirectory "$(themesPath)" -OutputReport $true'
        pwsh: true
      continueOnError: false
  ...
  - job: DetectHardcodedColors
    ...
    - task: PowerShell@2
      ...
      inputs:
        targetType: 'filePath'
        filePath: 'scripts/ui-analysis/detect-hardcoded-colors.ps1'
      ...
      script: |
        ...
        if ($changedFiles.Count -gt 0) {
          ...
          foreach ($file in $changedFiles) {
            ...
            & "scripts/theme-management/validate-theme-structure.ps1" -ThemeFile $file
            & "scripts/accessibility/validate-wcag-compliance.ps1" -ThemeFile $file -FailOnCritical $true
          }
        } else {
...
jobs:
  theme-structure:
    ...
    - name: Validate Theme Structure
      run: |
        ./scripts/theme-management/validate-theme-structure.ps1 -ThemeDirectory "Resources/Themes" -OutputReport $true
      shell: pwsh
  ...
  wcag-compliance:
    ...
    - name: WCAG Compliance Validation
      run: |
        ./scripts/accessibility/validate-wcag-compliance.ps1 -ThemeDirectory "Resources/Themes" -FailOnCritical $true
      shell: pwsh
  ...
  performance-testing:
    ...
    - name: Performance Testing
      run: |
        ./scripts/performance-optimization/performance-test-themes.ps1 -ThemeDirectory "Resources/Themes" -OutputReport $true
      shell: pwsh
  ...
  hardcoded-colors:
    ...
    - name: Detect Hardcoded Colors
      run: |
        ./scripts/ui-analysis/detect-hardcoded-colors.ps1 -ViewsDirectory "Views" -OutputReport $true
      shell: pwsh
...
pipeline {
    ...
    stages {
        ...
        stage('Theme Structure Validation') {
            steps {
                powershell '''
                    ./scripts/theme-management/validate-theme-structure.ps1 -ThemeDirectory $env:THEME_DIR -OutputReport $true
                '''
            }
            post {
                always {
        ...
        stage('WCAG Compliance') {
            steps {
                powershell '''
                    ./scripts/accessibility/validate-wcag-compliance.ps1 -ThemeDirectory $env:THEME_DIR -Standard $env:WCAG_STANDARD -FailOnCritical $true
                '''
        ...
        stage('Performance Testing') {
            steps {
                powershell '''
                    ./scripts/performance-optimization/performance-test-themes.ps1 -ThemeDirectory $env:THEME_DIR -OutputReport $true
                '''
